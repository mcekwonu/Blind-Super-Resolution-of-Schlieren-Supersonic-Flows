...
from realesrgan_x8 import Generator
from metrics import compute_niqe


def predict(input_dir, save_dir, model_path, device='cpu', upscale_factor=8, num_images=-1, verbose=True):
    """Inference using RealESRGAN x8 trained model
    ...
    for root, dirs, files in os.walk(input_dir):
        for idx, file in tqdm(
                enumerate(sorted(files)), desc="Running super resolution using RealESRGAN x8",
                total=len(files)
        ):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Super Resolution with RealESRGAN x8")
    parser.add_argument("--input_dir", "-i", type=str, required=True, help="Low resolution images directory")
