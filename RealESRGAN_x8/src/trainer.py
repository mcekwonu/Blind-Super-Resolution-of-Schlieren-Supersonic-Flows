...
class Trainer(nn.Module):
    """Train Real ESRGAN with pixel-loss, content-loss and adversarial loss.
    """
    ...
    def _resume(self, epoch, ckpt):
        ...
        try:
            ...
            date = datetime.now().strftime("%Y%m%d")
            print(f"\nResuming RealESRGAN x8 training from last checkpoint ({resume_epoch} epoch)...\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(prog="python trainer.py", description="Super Resolution RealESRGAN x8",
                                     usage="%(prog)s [options]")
    parser.add_argument("--train", action='store_true', help="Train RealESRGAN x8.[option] [filename]")
    parser.add_argument("--resume", action='store_true',
                        help="Resume/finetune RealESRGAN x8. [options] [filename, epoch, ckpt (date)]")
