...
class UNetDiscriminator(nn.Module):
    ...
    def forward(self, x):
        ...
        x3 = F.interpolate(x3, scale_factor=2, mode='bicubic', align_corners=False)
        x4 = F.leaky_relu(self.conv4(x3), negative_slope=0.2, inplace=True)

        if self.skip_connection:
            x4 = x4 + x2

        x4 = F.interpolate(x4, scale_factor=2, mode='bicubic', align_corners=False)
        x5 = F.leaky_relu(self.conv5(x4), negative_slope=0.2, inplace=True)

        if self.skip_connection:
            x5 = x5 + x1

        x5 = F.interpolate(x5, scale_factor=2, mode='bicubic', align_corners=False)
...
if __name__ == "__main__":
    ...
    netD = UNetDiscriminator(input_shape=(1, 16, 16))
