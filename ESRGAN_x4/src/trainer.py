...
class Trainer:
            ...
            for epoch in range(config.NUM_EPOCHS):
                for step, (lr, hr) in enumerate(self.get_loader):
                    ...
                    if iterations < config.WARMUP_ITERATIONS:
                        ...
                        print(
                            "[Epoch {}/{}] [Batch {}/{}]\t[G pixel: {:.5}]".format(
                                epoch + 1, config.NUM_EPOCHS, step, batches, pixel_loss.item())
                        )
                        continue
                    ...
                    if iterations % config.SAMPLE_INTERVAL == 0:
                        print(
                            ...
                            " content: {:.5}, adv: {:.5}, pixel: {:.5}]".format(
                                epoch, config.NUM_EPOCHS, iterations, loss_D.item(), loss_G.item(),
    ...
    def _resume(self, epoch, ckpt):
        ...
        Parameters:
        	...
        	epoch: (str) final epochs
        	ckpt: (str) saved checkpoints
        """
        try:
            ...
            date = datetime.now().strftime("%Y%m%d")
            print(f"\nResuming ESRGAN x4 training from last checkpoint with {resume_epoch} epoch...\n")
            start_timer = time.time()
    ...
    def get_loader(self):
        dataloader = DataLoader(
            SRDataset(root_dir=self.data_root), batch_size=config.BATCH_SIZE,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(prog="python trainer.py", description="Super Resolution ESRGAN x4",
                                     usage="%(prog)s [options]")
    parser.add_argument("--train", action='store_true', help="Train ESRGAN x4.")
    parser.add_argument("--resume", action='store_true', help="Resume/finetune ESRGAN x4. [options] [epoch, ckpt]")
    ...
    if opt.train:
        ...
        Trainer.resume_esrgan(epoch=EPOCH, ckpt=CHECKPOINT)
    else:
        parser.parse_args(["-h"])
