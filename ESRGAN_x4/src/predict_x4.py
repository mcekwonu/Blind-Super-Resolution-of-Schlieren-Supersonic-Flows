...
from esrgan_x4 import Generator
...
def predict(input_dir, save_dir, model_path, device='cpu', upscale_factor=4, num_images=-1, verbose=True):
    ...
    for root, dirs, files in os.walk(input_dir):
        for idx, file in tqdm(
                enumerate(sorted(files)), desc="Running super resolution using ESRGAN x4",
                total=len(files)
        ):
            ...
            with torch.no_grad():
                ...
                if verbose:
                    with open(f"{save_dir}/time_logs.txt", "w+") as f:
                        f.write(f"Time log for super resolution x4 of images for {os.path.basename(input_dir)}\n")
                        f.write(f"{'_' * 60}\n\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Super Resolution with ESRGAN x4")
