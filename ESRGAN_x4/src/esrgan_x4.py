...
class Generator(nn.Module):
    """Residual in Residual Dense Block Network.
        ...
        We extend ESRGAN for scale x4 and scale x8.

        Parameters:
            in_ch (int): Channel number of inputs. Default: 1
            out_ch (int): Channel number of out. Default: 1
...
class Discriminator(nn.Module):
    ...
    def __init__(self, input_shape=(1, 64, 64), upsample_factor=4):
        """
        Parameters:
        input_shape: dimension of input image.
        upsample_factor (int): value to obtain the right patch shape
        """
        super().__init__()
        in_channels, in_height, in_width = input_shape
        n = int(math.log(upsample_factor, 2))
        patch_h, patch_w = int(in_height / 4 ** n), int(in_width / 4 ** n)
    ...
    def forward(self, x):
        assert x.size(2) == self.input_size, f"Input size must be same as {self.input_size}, but got {x.size(2)}."
...
class VGGLoss(nn.Module):
    ...
    def __init__(self):
        ...
        #  ===== This portion is archived for some purposes and will d be deleted later ====== #
        # for params in vgg_19.parameters():
        #     params.requires_grad = False
        # layers = list(vgg_19.VGGFeaturesExtractor.children())[:35]
        # layers[0] = nn.Conv2d(1, 64, kernel_size=3, stride=1, padding=1)
        # self.VGGFeaturesExtractor = nn.Sequential(*layers)
        # ====================================================================================== #
...
if __name__ == "__main__":
    ...
    net = Generator(upsample_factor=4)
    x = torch.rand((1, 1, 16, 16))
    print(f"Generator output size: {net(x).size()}")

    x2 = torch.rand((1, 1, 64, 64))
    net = Discriminator(input_shape=(1, 64, 64), upsample=4)
    print(f"Discriminator output size: {net(x2).size()}")
