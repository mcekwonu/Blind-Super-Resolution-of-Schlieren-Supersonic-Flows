...
if __name__ == "__main__":
    ...
    data = SRDataset(root_dir='../../training_images_x4', resize=False)
    idx = np.random.randint(len(data))
    LR, HR = data[idx]
    print(f"Low resolution image size: {LR.shape}")
    print(f"High resolution image size: {HR.shape}")

    plt.imshow(HR.numpy().transpose(1, 2, 0), cmap="gray", vmin=HR.min(), vmax=HR.max())
