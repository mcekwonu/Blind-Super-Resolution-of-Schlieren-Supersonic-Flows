...
def ssim(sr, hr):
    ...
    window = np.dot(kernel, kernel.transpose())
...
def main(source_dir):
    ...
    imgpaths = (os.path.join(source_dir, f) for f in os.listdir(source_dir) if f.endswith("png"))
    imgpaths = sorted(imgpaths)
    target_dir = os.path.abspath(source_dir)
    scores = []

    for img_path in tqdm(imgpaths, desc="Computing NIQE score"):
